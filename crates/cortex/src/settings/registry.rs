...
pub const ALL: &[SettingDef] = &[
    ...
    SettingDef {
        key: "safety.fatal_fault_mask",
        label: "Fatal fault mask",
        description:
            "fault_sta bits that block motion; default excludes RS03 bit5 observed after Motor Tool recovery.",
        category: "safety",
        unit: None,
        value_kind: "u32",
        min: Some(0.0),
...
pub fn value_from_merged(s: &SafetyConfig, t: &TelemetryConfig, key: &str) -> Option<Json> {
    let v = match key {
        ...
        "safety.fatal_fault_mask" => json!(s.fatal_fault_mask),
