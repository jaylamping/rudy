...
- `src/main.rs` — `driver_node` CLI scaffold (ROS wiring comes next)
...
This package is also an `ament_cmake` package that runs `cargo build --release` during `colcon build` and installs `driver_node` into `lib/driver/`.
