...
- `src/main.rs` — `murphy_driver_node` CLI scaffold (ROS wiring comes next)

## Build (Rust only)

```bash
cd src/murphy_driver
cargo test
```
...
This package is also an `ament_cmake` package that runs `cargo build --release` during `colcon build` and installs `murphy_driver_node` into `lib/murphy_driver/`.
