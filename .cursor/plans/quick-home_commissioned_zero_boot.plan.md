...
todos:
  ...
  - id: home-all-real-implementation
    content: "Rewrite `crates/rudydae/src/api/home_all.rs` so it actually runs the homer instead of just calling `mark_homed`. Per-limb sequential, parallel across limbs (existing comment describes the intent). Each motor: call the same `slow_ramp::run` toward its `predefined_home_rad`. Per-limb failure isolation: if any motor in a limb aborts (HomeFailed / OffsetChanged), abandon the rest of THAT limb's sequence (don't drive distal joints when proximal is in unknown state) but continue running the other limbs in parallel. Return per-limb status in the response: { left_arm: 'completed', right_arm: 'quarantined: shoulder_pitch home_failed', ... }. Return SSE-style streaming progress as the existing endpoint already promises in its docstring."
    status: pending
  ...
  - id: contract-tests-commissioning
    content: "Tests for the new flash-persistent commissioning: (1) `commission_endpoint_writes_inventory` — mock CAN, POST commission, verify inventory.yaml gets the readback value and bumped commissioned_at; (2) `commission_endpoint_failure_leaves_inventory_clean` — force a CAN error after the SetZero send, verify inventory.yaml is NOT mutated and a clear error envelope is returned; (3) `set_zero_persists_to_flash` — verify the type-6 frame is followed by a type-22 frame on the wire (using the existing CAN frame capture pattern in `api_contract.rs`)."
    status: pending
