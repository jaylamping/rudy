...
Murphy is an upper-body humanoid using Robstride RS03 actuators, with **sim-first** development and **Isaac Lab** for sim-to-real. The onboard computer is a **Raspberry Pi 5** with a **Waveshare 2-CH CAN HAT**.
...
- **Monorepo**: single `murphy` workspace with `colcon` packages; cross-compile Rust/aarch64 in CI.
