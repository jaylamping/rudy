...
// proxied to a real `cortex`. By default we point at a local daemon
// (`127.0.0.1:8443`). To talk to the Rudy Pi over Tailscale instead, set
// VITE_CORTEX_URL in `link/.env.local` (or your shell), e.g.
//   VITE_CORTEX_URL=https://rudy.your-tailnet.ts.net:8443
...
export default defineConfig(({ mode }) => {
  ...
  const target = env.VITE_CORTEX_URL ?? "http://127.0.0.1:8443";
  ...
  return {
    ...
    test: {
      // jsdom for hooks that touch window/fetch/WebTransport.
      environment: "jsdom",
      globals: true,
